...
const DOMAIN = process.env.DOMAIN || '';                  // 域名或ip
...
const SETTINGS = {
    ['UUID']: UUID,              
    ['LOG_LEVEL']: 'none',       // 日志级别: none, info, warn, error
    ...
    ['CHUNK_SIZE']: 1024 * 1024, // 1024KB 数据块大小
...
async function read_vless_header(reader, cfg_uuid_str) {
    ...
    if (cmd !== COMMAND_TYPE_TCP) throw new Error(`unsupported command: ${cmd}`);
    ...
    if (atype === ADDRESS_TYPE_IPV4) {
        ...
        hostname = header.slice(idx, idx + 16).reduce((s, b2, i2, a) => i2 % 2 ? s.concat(((a[i2 - 1] << 8) + b2).toString(16)) : s, []).join(':');
    }
    ...
    return { hostname, port, data: header.slice(header_len), resp: new Uint8Array([version, 0]) };
}
...
function timed_connect(hostname, port, ms) {
    return new Promise((resolve, reject) => {
        ...
        const handle = setTimeout(() => reject(new Error(`connect timeout`)), ms);
        conn.on('connect', () => { clearTimeout(handle); resolve(conn); });
        conn.on('error', (err) => { clearTimeout(handle); reject(err); });
    });
}
    ...
    async function pump(src, dest, first_packet) {
        ...
        if (first_packet.length > 0) {
            if (dest.write) {
                ...
                try { await writer.write(first_packet); } finally { writer.releaseLock(); }
            }
        }
        try {
            if (src.pipe) {
            ...
            if (!err.message.includes('aborted')) log('error', 'Relay error:', err.message);
            throw err;
        }
    }
    return pump;
...
function socketToWebStream(socket) {
    let readController, writeController;
    socket.on('error', (err) => {
    ...
    return {
        readable: new ReadableStream({
            start(controller) {
                ...
                socket.on('data', (chunk) => { try { controller.enqueue(chunk); } catch (err) { log('error', 'Read controller error:', err.message); } });
                socket.on('end', () => { try { controller.close(); } catch (err) { log('error', 'Read controller close error:', err.message); } });
            },
        ...
        writable: new WritableStream({
            ...
            write(chunk) {
                return new Promise((resolve, reject) => {
                    if (socket.destroyed) return reject(new Error('Socket is destroyed'));
...
function relay(cfg, client, remote, vless) {
    ...
    const uploader = pump(client, remoteStream, vless.data).catch(err => {
        if (!err.message.includes('aborted') && !err.message.includes('socket hang up')) {
            log('error', `Upload error: ${err.message}`);
        }
    }).finally(() => client.reading_done && client.reading_done());
    const downloader = pump(remoteStream, client, vless.resp).catch(err => {
        if (!err.message.includes('aborted') && !err.message.includes('socket hang up')) {
...
class Session {
    ...
    async initializeVLESS(firstPacket) {
        ...
        try {
            ...
            const readable = new ReadableStream({
                start(controller) { controller.enqueue(firstPacket); controller.close(); }
            });
        ...
        try {
            ...
            while (this.pendingBuffers.has(this.nextSeq)) {
                ...
                if (!this.initialized && this.nextSeq === 0) {
                    if (!await this.initializeVLESS(nextData)) throw new Error('Failed to initialize VLESS connection');
            ...
            if (this.pendingBuffers.size > SETTINGS.MAX_BUFFERED_POSTS) throw new Error('Too many buffered packets');
            return true;
        } catch (err) {
    ...
    _startDownstreamResponse() {
        ...
        try {
            ...
            if (isH2) {
                ...
                const transform = new require('stream').Transform({
                    transform(chunk, encoding, callback) {
                        const size = 16384;
                        for (let i = 0; i < chunk.length; i += size) this.push(chunk.slice(i, i + size));
                        callback();
                    }
                });
            ...
            this.remote.on('end', () => { if (!this.currentStreamRes.writableEnded) this.currentStreamRes.end(); });
            this.remote.on('error', (err) => {
                log('error', `Remote error: ${err.message}`);
                if (!this.currentStreamRes.writableEnded) this.currentStreamRes.end();
            });
        } catch (err) {
    ...
    startDownstream(res, headers) {
        ...
        if (this.initialized && this.responseHeader) this._startDownstreamResponse();
        res.on('close', () => { log('info', 'Client connection closed'); this.cleanup(); });
        return true;
    }

    async _writeToRemote(data) {
        if (!this.remote || this.remote.destroyed) throw new Error('Remote connection not available');
        return new Promise((resolve, reject) => {
            this.remote.write(data, (err) => {
                if (err) { log('error', `Failed to write to remote: ${err.message}`); reject(err); } else resolve();
            });
        });
    }
    ...
    cleanup() {
        if (!this.cleaned) {
            ...
            if (this.remote) { this.remote.destroy(); this.remote = null; }
            this.initialized = false;
            this.headerSent = false;
        }
    }
    ...
    if (req.method === 'POST' && seq !== null) {
        ...
        req.on('data', chunk => {
            ...
            if (size > SETTINGS.MAX_POST_SIZE) {
                if (!headersSent) { res.writeHead(413); res.end(); headersSent = true; }
                return;
            }
            data.push(chunk);
        });
        ...
        req.on('end', async () => {
            ...
            try {
                ...
                if (!headersSent) { res.writeHead(200, headers); headersSent = true; }
                res.end();
            } catch (err) {
                log('error', `Failed to process POST request: ${err.message}`);
                session.cleanup();
                sessions.delete(uuid);
                if (!headersSent) { res.writeHead(500); headersSent = true; }
                res.end();
            }
        });
        return;
